...
function api_create_theme(name) {
  return new Promise((fulfill, reject) => {
    fetch("/api/theme/create", {
      ...
      .then((response) => {
        ...
        response.json().then((res) => fulfill(res));
      })
      .catch(reject);
  });
}
...
function api_config_sensor_remove(name, module) {
  return new Promise((fulfill) => {
    fetch("/api/config_sensor_remove", {
      method: "POST",
      headers: { "Content-Type": "application/json" },

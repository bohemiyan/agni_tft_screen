...
function next_update_region(context, state, config, fulfill) {
  ...
  const _image = context.getImageData(
    _rect.x,
    _rect.y,
    _rect.width,
    _rect.height,
  ...
  return new Promise((fulfill) => {
    if (state.update_orientation) {
      ...
      if ("redraw" === theme.refresh || state.pending_redraw(state)) {
        ...
        lcd_redraw(
          state,
          context.getImageData(0, 0, config.canvas.width, config.canvas.height),
        );
...
function next_draw_widgets(
  ...
  if (index < total) {
    ...
    if (_widget_config.refresh && _widget_config.sensor) {
      ...
      if (_sensor) {
        _sensor_reading = _sensor.sample(
          _widget_config.refresh,
          _widget_config.format,
        );
      }
    }
    ...
    return _sensor_reading.then((sensor) => {
      ...
      _draw_promise.then((changed) => {
        if (!state.drawing && changed) {
          calc_update_region(
            fix_rect_bounds(config, _widget_config.rect),
          ).forEach((each) => {
...
function poll_inactive_screen_sensors(state, screens, index, active, fulfill) {
  if (index < screens.length) {
    ...
    return poll_inactive_screen_sensors(
      state,
      screens,
      1 + index,
      active,
...
function start_draw_canvas(state, config, theme) {
  ...
  poll_inactive_screen_sensors(state, theme.screens, 0, _active_screen, () => {
    ...
    draw_screen(_context, state, config, _active_screen).then(() => {
      // update lcd screen
      update_device_screen(_context, state, config, theme).then(
        (device_updated) => {
          state.output_context.putImageData(
            _context.getImageData(
              0,
              0,
              config.canvas.width,
              config.canvas.height,
...
function init_web_gui(state, config, theme) {
  return new Promise((fulfill) => {
    ...
    _web.use(express.static(path.join(__dirname, "../client/dist")));
...
function load_panel_font(font) {
  return new Promise((fulfill) => {
    ...
    file_exists(_full_path).then((found) => {
      if (found) {
        logger.info(
          "initialize: register " + _full_path + " as " + font.face.family,
        );
...
function register_fonts() {
  return new Promise((fulfill) => {
    const _fonts = [
      ...
      {
        path: "/truetype/liberation/LiberationSans-Regular.ttf",
        face: { family: "Arial", weight: "normal", style: "normal" },
      },
      {
        path: "/truetype/liberation/LiberationSans-Bold.ttf",
        face: { family: "Arial", weight: "bold", style: "normal" },
      },
      {
        path: "/truetype/liberation/LiberationSans-Italic.ttf",
        face: { family: "Arial", weight: "normal", style: "italic" },
      },
      {
        path: "/truetype/liberation/LiberationSans-BoldItalic.ttf",
        face: { family: "Arial", weight: "bold", style: "italic" },
      },

      // Courier New → Liberation Mono
      {
        path: "/truetype/liberation/LiberationMono-Regular.ttf",
        face: { family: "Courier New", weight: "normal", style: "normal" },
      },
      {
        path: "/truetype/liberation/LiberationMono-Bold.ttf",
        face: { family: "Courier New", weight: "bold", style: "normal" },
      },
      {
        path: "/truetype/liberation/LiberationMono-Italic.ttf",
        face: { family: "Courier New", weight: "normal", style: "italic" },
      },
      {
        path: "/truetype/liberation/LiberationMono-BoldItalic.ttf",
        face: { family: "Courier New", weight: "bold", style: "italic" },
      },

      // Times New Roman → Liberation Serif
      {
        path: "/truetype/liberation/LiberationSerif-Regular.ttf",
        face: { family: "Times New Roman", weight: "normal", style: "normal" },
      },
      {
        path: "/truetype/liberation/LiberationSerif-Bold.ttf",
        face: { family: "Times New Roman", weight: "bold", style: "normal" },
      },
      {
        path: "/truetype/liberation/LiberationSerif-Italic.ttf",
        face: { family: "Times New Roman", weight: "normal", style: "italic" },
      },
      {
        path: "/truetype/liberation/LiberationSerif-BoldItalic.ttf",
        face: { family: "Times New Roman", weight: "bold", style: "italic" },
      },

      // Verdana → DejaVu Sans
      {
        path: "/truetype/dejavu/DejaVuSans.ttf",
        face: { family: "Verdana", weight: "normal", style: "normal" },
      },
      {
        path: "/truetype/dejavu/DejaVuSans-Bold.ttf",
        face: { family: "Verdana", weight: "bold", style: "normal" },
      },

      // Georgia → DejaVu Serif
      {
        path: "/truetype/dejavu/DejaVuSerif.ttf",
        face: { family: "Georgia", weight: "normal", style: "normal" },
      },
      {
        path: "/truetype/dejavu/DejaVuSerif-Bold.ttf",
        face: { family: "Georgia", weight: "bold", style: "normal" },
      },

      // Garamond → EB Garamond
      {
        path: "/opentype/ebgaramond/EBGaramond12-Regular.otf",
        face: { family: "Garamond", weight: "normal", style: "normal" },
      },
      {
        path: "/opentype/ebgaramond/EBGaramond12-Bold.otf",
        face: { family: "Garamond", weight: "bold", style: "normal" },
      },
      {
        path: "/opentype/ebgaramond/EBGaramond12-Italic.otf",
        face: { family: "Garamond", weight: "normal", style: "italic" },
      },

      // Helvetica → FreeSans from GNU FreeFont
      {
        path: "/truetype/freefont/FreeSans.ttf",
        face: { family: "Helvetica", weight: "normal", style: "normal" },
      },
      {
        path: "/truetype/freefont/FreeSans-Bold.ttf",
        face: { family: "Helvetica", weight: "bold", style: "normal" },
      },
    ];
...
function main() {
  ...
  const _config_file =
    _args.length > 0 ? _args[0] : path.join(home_dir, "config.json"); // {dir}/config.json

  logger.info("config " + _config_file);

  load_config(_config_file).then(
    (config) => {
      ...
      load_config(_theme_file).then(
        (theme) => {
          register_fonts().then(() => {
            const _output_canvas = node_canvas.createCanvas(
              config.canvas.width,
              config.canvas.height,
            );
            const _canvas1 = node_canvas
              .createCanvas(config.canvas.width, config.canvas.height)
              .getContext("2d", { pixelFormat: config.canvas.pixel });
            const _canvas2 = node_canvas
              .createCanvas(config.canvas.width, config.canvas.height)
              .getContext("2d", { pixelFormat: config.canvas.pixel });
            ...
            const _state = {
              ...
              output_context: _output_canvas.getContext("2d", {
                pixelFormat: config.canvas.pixel,
              }),
              active_context: 0,
              ...
              led_thread: new threads.Worker("./led_thread.js", {
                workerData: config.led_config,
              }),
              lcd_thread: new threads.Worker("./lcd_thread.js", {
                workerData: {
              ...
              done_redraw: (state) =>
                state.redraw_count < state.redraw_want
                  ? state.redraw_count++
                  : state.redraw_count,
            };
            ...
            initialize(_state, config, theme).then(
              () => {
                ...
                if (_screen.led_config) {
                  ...
                  config.led_config.intensity =
                    _screen.led_config.intensity || 3;
                  config.led_config.speed = _screen.led_config.speed || 3;
                }
